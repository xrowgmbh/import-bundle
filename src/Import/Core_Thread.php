...
class Core_Thread {
    ...
    private $errors = array(
        Core_Thread::FUNCTION_NOT_CALLABLE   => 'You must specify a valid function name that can be called from the current scope.',
        Core_Thread::COULD_NOT_FORK          => 'pcntl_fork() returned a status of -1. No new process was created',
    );
    ...
    public function setRunnable( $_runnable, $_object = null ) {
        if( self::runnableOk( $_runnable, $_object ) ) {
            ...
            throw new \Exception( $this->getError( Core_Thread::FUNCTION_NOT_CALLABLE ), Core_Thread::FUNCTION_NOT_CALLABLE );
        }
    }
   ...
   public static function runnableOk( $_runnable ) {
    if (is_array($_runnable) && count($_runnable) == 2 ) {
        return method_exists($_runnable[0], $_runnable[1]);
    }
    ...
    public function start() {
        $pid = pcntl_fork();
        if( $pid == -1 ) {
            throw new Core_Exception( $this->getError( Core_Thread::COULD_NOT_FORK ), Core_Thread::COULD_NOT_FORK );
        }
        if( $pid ) {
            // parent
            $this->pid = $pid;
